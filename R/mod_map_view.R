  ...
  tagList(
    ...
    box(
      ...
      column(
        ...
        box(
          id = ns("box_phaplo"), width = 12, solidHeader = FALSE, collapsible = TRUE, collapsed = TRUE, status = "primary", title = "Parents haplotypes table",
...
mod_map_view_server <- function(input, output, session,
  ...
  observeEvent(input$goPar, {
    ...
    # select specific tab
    updateTabsetPanel(
      session = parent_session, inputId = "Map_tabset",
      selected = "Map_par"
    )
    # expand specific box
    updateBox(id = "Map_box", action = "toggle", session = parent_session)
  })
