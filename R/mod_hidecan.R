  ...
  tagList(
    fluidRow(
      ...
      column(
        ...
        fluidRow(
          column(
            6,
            box(
              width = 12, solidHeader = TRUE, status = "info", title = "Select dataset *",
          ...
          column(
            6,
            box(
              width = 12, solidHeader = TRUE, status = "info", title = "Define thresholds *",
      ...
      column(
        12,
        box(
          width = 12, solidHeader = TRUE, status = "info", collapsible = TRUE, collapsed = TRUE, title = "HIDECAN plot options",
...
mod_hidecan_view_server <- function(input, output, session,
  ...
  observe({
    if (!is.null(loadHidecan()$GWASpoly)) {
      ...
      max_score <- max(sapply(loadHidecan()$GWAS, function(x) {
        max(x$score, na.rm = TRUE)
      }))
    ...
    if (length(loadHidecan()$DE)) {
      max_score <- max(sapply(loadHidecan()$DE, function(x) {
        max(x$score, na.rm = TRUE)
      }))
      max_log2fc <- max(sapply(loadHidecan()$DE, function(x) {
        max(abs(x$log2FoldChange), na.rm = TRUE)
      }))
  ...
  make_names_hidecan_data <- function(hidecan_list) {
    data_type_labels <- c(
      "GWAS_data_thr" = "GWAS data",
      "DE_data_thr" = "DE data",
      "CAN_data_thr" = "Candidate genes list"
    )

    labels <- sapply(hidecan_list, function(x) {
      class(x)[[1]]
    })
  ...
  make_placeholders_hidecan_data <- function(hidecan_list) {
    data_type_labels <- c(
      "GWAS_data_thr" = "[GWAS peaks]",
      "DE_data_thr" = "[DE genes]",
      "CAN_data_thr" = "[Candidate genes]"
    )

    labels <- sapply(hidecan_list, function(x) {
      data_type_labels[class(x)[[1]]]
    }) |>
      unname()
  ...
  observe({
    ...
    updateTextInput(
      inputId = "data_names",
      label = paste0(
        "Add custom prefix for your ", length(hidecan_data()[[1]]),
        " tracks."
      ), value = NULL, placeholder = make_placeholders_hidecan_data(hidecan_data()[[1]])
    )
  ...
  plotHeight <- reactive({
    ...
    if (!(is.null(input$height) | is.na(input$height))) {
      ...
      size <- (n.ytracks * n.chr / input$ncols) * 240
    }
  ...
  fn_download <- function() {
    if (input$fformat != "RData") {
      ...
      save(hidecan_data(), file = fn_downloadname())
    }
  }
